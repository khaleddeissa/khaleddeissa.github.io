...
const icons: Record<string, string> = {
  ...
  reset:
    '<path d="M3 10a9 9 0 0 1 15-6l3 3M21 3v4h-4M21 14a9 9 0 0 1-15 6l-3-3M3 21v-4h4"/>',
    ...
    for (const source of sources) {
      ...
      try {
        ...
        viewport.setAttribute(
          "aria-label",
          "Architecture diagram. Drag to move, pinch to zoom, or use arrow keys and plus or minus.",
        );
        ...
        function reset() {
          ...
          y = 0;
          paint();
          status.textContent = "Diagram fitted to view";
        }
        ...
        button(navigation, "Move up", "up", () => navigate(0, -60), "up");
        button(navigation, "Move left", "left", () => navigate(-60, 0), "left");
        ...
        button(navigation, "Move down", "down", () => navigate(0, 60), "down");
        button(
          navigation,
          "Zoom in",
          "plus",
        ...
        button(
          ...
          async () => {
            try {
              ...
              status.textContent = "Mermaid source copied";
            } catch {
              status.textContent =
                "Open View Mermaid source to copy the diagram";
            }
          },
          "copy",
        );
        const footer = document.createElement("div");
        footer.className = "mermaid-footer";
        const hint = document.createElement("span");
        hint.textContent = "Drag to move · Pinch or use + / − to zoom";
        ...
        const encoded = btoa(
          Array.from(new TextEncoder().encode(state), (byte) =>
        ...
        live.textContent = "Edit in Mermaid Live ↗";
        footer.append(hint, live);
        const details = document.createElement("details");
        const summary = document.createElement("summary");
        summary.textContent = "View Mermaid source";
        source.replaceWith(panel);
        source.classList.remove("mermaid");
        details.append(summary, source);
        stage.append(viewport, tools, navigation);
        panel.append(stage, footer, details, status);
        reset();
        ...
        viewport.addEventListener("keydown", (event) => {
          const actions: Record<string, () => void> = {
            ArrowUp: () => navigate(0, -60),
            ArrowDown: () => navigate(0, 60),
            ArrowLeft: () => navigate(-60, 0),
            ArrowRight: () => navigate(60, 0),
            "+": () => changeZoom(zoom * 1.4),
            "=": () => changeZoom(zoom * 1.4),
            "-": () => changeZoom(zoom / 1.4),
            "0": reset,
        ...
        delete source.dataset.rendering;
      }
    }
  } catch (error) {
    sources.forEach((source) => {
